...
class GoToGoal(Task):
    ...
    def build_world_config(self, layout: dict, rs: np.random.RandomState) -> dict:
        return {
            "bodies": {
                "goal": po.get_goal(
                    "goal", self.GOAL_SIZE, layout["goal"], utils.random_rot(rs)
                )
            }
        }
    ...
    def _resample_goal_position(
        self, layout: dict, placements: dict, rs: np.random.RandomState
    ):
        layout.pop("goal")
        for _ in range(10000):
            goal_xy = utils.draw_placement(
                rs, None, self.placement_extents, self.GOAL_KEEPOUT
            )
