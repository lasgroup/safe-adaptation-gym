...
def get_vase(
    ...
    xy: np.ndarray,
    rot: float,
    color: np.ndarray = c.VASES_COLOR,
...
def get_gremlin(
    ...
    xy: np.ndarray,
    rot: float,
    color: np.ndarray = c.GREMLINS_COLOR,
) -> Tuple[Iterable[str], str]:
    gremlin_obj_dict = {
        "name": name,
        "size": np.ones(3) * size,
...
def get_hazard(
    ...
    hazard_dict = {
        ...
        "pos": np.r_[xy, 2.0 * c.HAZARDS_HEIGHT],
        "quat": utils.rot2quat(rot),
        "type": "cylinder",
        "contype": 0,
        "conaffinity": 0,
...
def get_pillar(
    ...
    xy: np.ndarray,
    rot: float,
    color: np.ndarray = c.PILLARS_COLOR,
) -> Tuple[Iterable[str], str]:
    pillar_dict = {
        "name": name,
